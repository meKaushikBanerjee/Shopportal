...
function addDate( $date )
{
    ...
    $this->Cell(10,5, "Order Date", 0, 0, "C");
...
function addClientBillingAdresse( $adresse )
{
	$this->SetLineWidth(0.1);
	$this->SetFillColor(192);
	$this->SetFont('Arial','',16);
	$this->RoundedRect(20, 38, 80, 35, 3.5, 'DF');
    $r1     = $this->w - 188;
    $r2     = $r1 + 68;
    $y1     = 40;
    $this->SetXY( $r1, $y1);
    $this->SetFont( "Arial", "B", 8);
    $this->MultiCell( 200, 6, $adresse);
}

// Client Billing address
function addClientShippingAdresse( $adresse )
{
	$this->SetLineWidth(0.1);
	$this->SetFillColor(192);
	$this->SetFont('Arial','',16);
	$this->RoundedRect(108, 38, 80, 35, 3.5, 'DF');
    $r1     = $this->w - 100;
    $r2     = $r1 + 68;
    $y1     = 40;
    $this->SetXY( $r1, $y1);
...
function addEcheance( $date )
{
    ...
    $this->SetXY( $r1 + ($r2 - $r1)/2 - 5 , $y1+1 );
    $this->SetFont( "Arial", "B", 10);
    $this->Cell(10,4, "Order Date-Time", 0, 0, "C");
    $this->SetXY( $r1 + ($r2-$r1)/2 - 5 , $y1 + 5 );
    $this->SetFont( "Arial", "", 10);
    $this->Cell(10,5,$date, 0,0, "C");
}
...
/*function addCadreEurosFrancs($sc,$cd,$tp)
{
    $r1  = $this->w - 70;
    $r2  = $r1 + 60;
    $y1  = $this->h - 100;
    ...
    $this->SetFont( "Arial", "B", 8);
    $this->SetXY( $r1+22, $y1 );
    $this->Cell(30,4, "Amount", 0, 0, "C");
    $this->SetFont( "Arial", "B", 6);
    $this->SetXY( $r1, $y1+5 );
    $this->Cell(20,4, "Shipping Charge", 0, 0, "C");
    $this->SetXY( $r1, $y1+10 );
    $this->Cell(20,4, "Coupon Discount", 0, 0, "C");
    $this->SetXY( $r1, $y1+15 );
    $this->Cell(20,4, "Total Payable Amount", 0, 0, "C");
}
...
function temporaire( $texte )
{
    ...
    $this->Text(5,190,$texte);
    $this->Rotate(0);
    $this->SetTextColor(0,0,0);
}

function deliverystatus( $num )
{
    $this->SetFont('Arial','B',50);
    $this->SetTextColor(203,203,203);
    $this->Rotate(45,55,190);
    if($num==4)
    {
    	$this->Text(10,200,"Order Delivered");
    }
    elseif($num==6)
    {
    	$this->Text(-10,220,"Order Cancelled by seller");
    }
    elseif($num==7)
    {
    	$this->Text(-10,220,"Order Cancelled by customer");
    }    
    $this->Rotate(0);
    $this->SetTextColor(0,0,0);
}
